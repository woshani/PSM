...
<center><h2 class="title">REKOD STATISTIK LAPORAN KEHADIRAN HARIAN </br> <b><%=instituition.getAcademic_name()%></b> </br> (<b><%=month %></b> ) </h2></center>
		...
		<table width="1000px" border="0" cellpadding="5">
				...
				<%
					...
					for(Instituition instituitionss : instituitions){
						ppdAttendance = controller.jpnPpdReportRecord(instituitionss.getPpd(), date);
						count = count + 1;	
						%>
						<tr>	
							<td><%=count %></td>
							<td><a href="PPDKedatanganSekolahHistory.jsp?instituitionId=<%=instituitionss.getAcademic_instituition_id()%>&month=<%=month%>" ><%=instituitionss.getAcademic_name() %></a></td>
...
<%
	...
	ppdAttendance = controller.jpnReportRecord(city, date);
