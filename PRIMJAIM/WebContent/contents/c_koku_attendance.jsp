...
<%@ page import="bean.KokuAttendance"%>
...
<%
String instKokuId = request.getParameter("instKokuId");
ControllerWrapper controller = new ControllerWrapper();
List<StudentKoku> studentKokus = controller.getAllStudentByInstKokuId(instKokuId);
String idMeet= request.getAttribute("theName").toString();
%>


<div class="entry">
<h3>Kehadiran Pelajar Koku</h3>
<h6>*Pilih nama pelajar yang tidak hadir sahaja</h6>
<form method="post" action="TakeKokuAttendance" onSubmit="return selIt();">
	<input type="hidden" name="idMeeting" value="<%=idMeet%>" />
<table>
	<tr>
		<td>
			<select NAME="SelectList" ID="SelectList" SIZE="10"
					multiple="multiple" style="width: 300px">
				<% for(StudentKoku studentKoku : studentKokus) {  %>
					<option value='<%=studentKoku.getStudentId().getStudent_id()%>'><%=studentKoku.getStudentId().getName()%></option>
				<%}%>
			</select>			
		</td>
		<td>
		...
		<td>
			<select name="PickList" id="PickList" SIZE="10"
					multiple="multiple" style="width: 300px">
			</select>
		</td>
	</tr>
	<tr>
		...
		<td></td>
		<td ALIGN="left"><br />
			<input  type="submit" name="tidakHadir" value="Kemaskini" 
				style="background-color:#9ACD32; height: 30px; width: 100px; 
					border: solid 1px #6CC417; border-radius:8px; color:#FFFFFF;">
		</td>
	</tr>
</table>
</form>
