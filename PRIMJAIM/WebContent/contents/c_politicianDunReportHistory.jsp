		...
		<table width="1000px" border="0" cellpadding="5">
			...
			<%
				...
				for (Instituition instituitions : instituitionss) {
					PpdAttendance ppdAttendance = controller.ppdReportRecord(
							instituitions.getAcademic_instituition_id(), date);
					count = count + 1;
			%>
			<tr>
				<td><%=count%></td>
				<td><a
					href="politicianAttendanceStatusHistory.jsp?instituitionId=<%=instituitions.getAcademic_instituition_id()%>&month=<%=month%>"><%=instituitions.getAcademic_name()%></a></td>
